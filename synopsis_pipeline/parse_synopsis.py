...
def main() -> int:
    ...
    default_output = str(config.synopsis_parsed_results_path)
    default_input = str(config.synopsis_raw_data_path)
    ...
    parser.add_argument(
        ...
        help=f"Input raw JSON (default: {default_input})",
    )
    parser.add_argument(
        "--session-file",
        default=default_session,
        help=f"Session state file (default: {default_session})",
    )
    parser.add_argument(
        "--output",
        default=default_output,
        help=f"Output parsed results JSON (default: {default_output})",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=llm_config.get("batch_size", 45),
        help=f"Batch size for processing (default: {llm_config.get('batch_size', 45)})",
    )
    ...
    # Print configuration
    print("=" * 80)
    print("BC FRESHWATER FISHING REGULATIONS - SYNOPSIS PARSING")
    print("=" * 80)

    if not (args.export_session or args.archive_current):
        print("\n📝 Input:")
        if args.file:
            print(f"  Raw data JSON: {args.file}")
        if args.resume:
            print(f"  Session state: {args.session_file} (RESUMING)")

        print("\n💾 Output:")
        print(f"  Parsed regulations: {args.output}")
        print(f"  Session state: {args.session_file}")
    ...
    if is_resuming:
        ...
        # Add previously failed items for retry
        failed_count = len(session.failed_items)
        validation_failed_count = len(session.validation_failures)
        ...
        # Reset retry counts for all items being retried (fresh start)
        if failed_count > 0 or validation_failed_count > 0:
            print(
                f"Giving {failed_count + validation_failed_count} previously failed items a fresh retry count..."
            )
        ...
        # Clear failed items list - they're getting another chance
    ...
    print(f"  Failed (will retry on resume): {len(session.failed_items)}")
    print(f"  Failed validation: {len(session.validation_failures)}")

    if session.failed_items:
        print(f"\n  Failed Items (resume to retry with fresh retry count):")
