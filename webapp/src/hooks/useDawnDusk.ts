 ...
 *
 * The caller (Map.tsx) is responsible for calling `updatePosition` from its
 * own map event listeners (initial load + `moveend`); this hook owns only
 * the threshold/date-change gating and the periodic date-rollover check.
 */
export function useDawnDusk(): {
    ...
    // Safety-net: catch a date rollover during a long-open session even if
    // the map never moves.
    useEffect(() => {
