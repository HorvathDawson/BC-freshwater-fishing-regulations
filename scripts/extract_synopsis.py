...
class FishingSynopsisParser:
    def __init__(self, output_dir="output", debug_dir="debug_crops", audit_dir="debug_frames"):
        ...
        self.MAX_SYMBOL_DIM = 25.0 # Max size for a fish icon
        self.MAP_REJECTION_DIM = 35.0 # Anything larger is likely a map/boundary
    ...
    def process_column_text(self, text, is_regs=False):
        ...
        return "\n".join(cleaned_lines).strip(), symbols, ", ".join(mu_list)
...
def print_pretty_table(data):
    ...
    for row in data:
        ...
        m_l = smart_wrap(row['mu'], width=m_w) or [""]
        r_l = smart_wrap(row['regs'], width=r_w) or [""]
