#!/usr/bin/env python3
"""
DEPRECATED: This script has been integrated into regulation_pipeline.py

Please use the regulation_pipeline module with --map-only and --verbose flags:

    # Show detailed statistics (replaces old test_mapper)
    python -m regulation_mapping.regulation_pipeline --map-only --verbose

    # Run full pipeline with verbose output
    python -m regulation_mapping.regulation_pipeline --verbose
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"Path to parsed regulations JSON file (default: {config.synopsis_parsed_results_path})",
    )
    parser.add_argument(
        "--gazetteer",
        type=Path,
        default=config.fwa_metadata_path,
        help=f"Path to stream metadata pickle file (default: {config.fwa_metadata_path})",
    )
    parser.add_argument(
        "--graph",
        type=Path,
        default=config.fwa_graph_path,
        help=f"Path to FWA graph pickle file for tributary enrichment (default: {config.fwa_graph_path})",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    output_dir = Path("output/fwa/regulation_index")
