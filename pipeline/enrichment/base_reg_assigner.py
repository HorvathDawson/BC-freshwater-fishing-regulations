...
def _precompute_mu_features(
    ...
    for wsc, pairs in wsc_groups.items():
    ...
    logger.info(
        "  Two-pass MU-stream query: %d streams × %d MUs …",
        len(all_geoms),
        len(mu_ids_list),
    )
...
def _filter_and_assign(
    ...
    *,
    ...
    if fids and _feature_type_matches("stream", reg.feature_types):
        count += assignments.assign_fids_bulk(
            fids,
            reg.reg_id,
            phase=4,
            is_admin=is_admin,
    ...
    if wbks:
        ...
        if _feature_type_matches("lake", reg.feature_types):
            ...
            if lake_wbks:
                count += assignments.assign_wbks_bulk(
                    lake_wbks,
                    reg.reg_id,
                    phase=4,
                    is_admin=is_admin,
...
def assign_base_regulations(
    ...
    for reg in tqdm(base_regs, desc="  Phase 4: base regs", leave=False):
        ...
        if reg.zone_ids:
            ...
            # Derive region label so the webapp can group zone regs properly
            reg_info["region"] = "Region " + ", ".join(sorted(reg.zone_ids))
            # Expose zone IDs so reach builder can populate reach["z"]
            reg_info["zone"] = ", ".join(sorted(reg.zone_ids))
        if reg.dates:
