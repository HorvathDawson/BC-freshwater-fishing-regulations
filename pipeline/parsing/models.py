...
def validate_batch(
    ...
    for i, (entry, row) in enumerate(zip(entries, input_rows)):
        ...
        # Water name itself can indicate a tributary entry
        # e.g. "BUTTLE LAKE'S TRIBUTARIES", "WHITESWAN LAKE'S INLET & OUTLET STREAMS"
        has_trib_name = (
            "tributar" in water_lower
            or "inlet" in water_lower
            or "outlet" in water_lower
        )
        ...
        if (
            entry.includes_tributaries
            and not has_trib_symbol
            and not has_trib_text
            and not has_trib_name
        ):
            row_errors.append(
