...
@pytest.fixture(scope="module")
def base_entry_names() -> set:
    """Load base entry names from match_table.json."""
    if not _MATCH_TABLE_PATH.exists():
        return set()
    with open(_MATCH_TABLE_PATH, encoding="utf-8") as f:
        data = json.load(f)
    return {d["criteria"]["name_verbatim"] for d in data}
...
class TestOverridesDeserialization:
    ...
    def test_all_entries_deserialise(self, raw_overrides: List[Dict[str, Any]]) -> None:
    ...
    def test_round_trip(self, overrides: List[OverrideEntry]) -> None:
        ...
        for i, entry in enumerate(overrides):
            ...
            assert (
                rebuilt.criteria.name_verbatim == entry.criteria.name_verbatim
...
class TestOverrideBusinessRules:
    ...
    def test_no_duplicate_criteria_keys(self, overrides: List[OverrideEntry]) -> None:
        """No two entries should share the same (region, name, mus) key."""
        seen: Counter = Counter()
        for e in overrides:
            key = (
                e.criteria.region,
                e.criteria.name_verbatim,
                tuple(sorted(e.criteria.mus)),
            )
            seen[key] += 1
        dupes = {k: v for k, v in seen.items() if v > 1}
        assert not dupes, f"{len(dupes)} duplicate criteria keys:\n" + "\n".join(
            f"  {k}: {v}x" for k, v in dupes.items()
        )

    def test_criteria_name_not_empty(self, overrides: List[OverrideEntry]) -> None:
        for i, e in enumerate(overrides):
            assert (
                e.criteria.name_verbatim.strip()
            ), f"Entry {i} has empty name_verbatim"

    def test_criteria_region_valid(self, overrides: List[OverrideEntry]) -> None:
        ...
        assert not bad, f"{len(bad)} entries with unknown region:\n" + "\n".join(bad)
    ...
    def test_mu_format(self, overrides: List[OverrideEntry]) -> None:
        ...
        assert not bad, f"{len(bad)} entries with malformed MU codes:\n" + "\n".join(
            bad
        )

    def test_skip_entries_have_reason(self, overrides: List[OverrideEntry]) -> None:
...
class TestOverrideFieldIntegrity:
    ...
    def test_gnis_ids_are_numeric_strings(self, overrides: List[OverrideEntry]) -> None:
        bad = []
        for i, e in enumerate(overrides):
            for gid in e.gnis_ids:
                if not gid.isdigit():
                    bad.append(f"  [{i}] {e.criteria.name_verbatim}: gnis_id={gid!r}")
        assert not bad, f"{len(bad)} non-numeric gnis_ids:\n" + "\n".join(bad)
    ...
    def test_admin_targets_structure(self, overrides: List[OverrideEntry]) -> None:
    ...
    def test_all_entries_have_type_override(
        ...
        bad = [i for i, d in enumerate(raw_overrides) if d.get("type") != "override"]
        assert not bad, f"Entries without type='override': indices {bad}"

    def test_variant_of_resolves(
        self, overrides: List[OverrideEntry], base_entry_names: set
    ) -> None:
        """Every variant_of must point to a non-skip override or base entry."""
        non_skip_override_names = {
            e.criteria.name_verbatim for e in overrides if not e.skip
        }
        all_target_names = non_skip_override_names | base_entry_names
        bad = []
        for i, e in enumerate(overrides):
            if e.variant_of and e.variant_of.name_verbatim not in all_target_names:
                bad.append(
                    f"  [{i}] {e.criteria.name_verbatim}: "
                    f"variant_of={e.variant_of!r} not found"
                )
        assert not bad, f"{len(bad)} variant_of targets unresolved:\n" + "\n".join(bad)

    def test_variant_of_only_on_skip_entries(
        self, overrides: List[OverrideEntry]
    ) -> None:
        """variant_of should only appear on skip=True entries."""
        bad = []
        for i, e in enumerate(overrides):
            if e.variant_of and not e.skip:
                bad.append(
                    f"  [{i}] {e.criteria.name_verbatim}: "
                    f"has variant_of but skip=False"
                )
        assert not bad, f"{len(bad)} non-skip entries with variant_of:\n" + "\n".join(
            bad
        )
