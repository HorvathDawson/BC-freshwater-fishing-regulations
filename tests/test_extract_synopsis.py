...
from synopsis_pipeline.extract_synopsis import FishingSynopsisParser
...
def raw_regs_contain(row, text):
    """
    Helper to search for text within the raw regulation t
...
EXPECTED_FIRST_ROWS = {
    ...
    19: {
        "water": "LITTLE QUALICUM RIVER",
        "mu": "1-6",
        "regs_contains": "Fly fishing only",
    },
...
EXPECTED_LAST_ROWS = {
    ...
    60: {
        "water": "ZYMOETZ (Copper) RIVER",
        "mu": "6-9",
        "regs_contains": "McDonell Lake",
    },
}
...
def test_symbol_logic(parser, pdf, page_num, symbol_checks):
    ...
    for check in symbol_checks:
        ...
        assert (
            len(matching_rows) > 0
        ), f"Could not find {check['water']} on page {page_num}"

        actual_symbols = matching_rows[0].symbols
        for sym in check["has"]:
            assert (
                sym in actual_symbols
            ), f"{check['water']} missing symbol {sym}. Got: {actual_symbols}"
...
def test_columbia_river_multiple_mus(parser, pdf):
    ...
    assert isinstance(
        columbia.mu, list
    ), f"Expected list for 'mu', got {type(columbia.mu)}"
    ...
    assert len(columbia.mu) == len(
        expected_mus
    ), f"Extracted unexpected MUs. Got: {columbia.mu}, Expected: {expected_mus}"


def test_embedded_mu_in_waterbody_name(parser, pdf):
    """
    Test that MUs appearing as suffix in waterbody names are properly extracted.
    Example: "COPPER CREEK 6-12" should extract as water="COPPER CREEK", mu=["6-12"]
    """
    # Page 17 is index 16
    page = pdf.pages[16]
    result = parser.extract_rows(page)
    ...
    # Find COPPER CREEK (should not have 6-12 in the name)
    copper = next((r for r in rows if "COPPER CREEK" in r.water), None)
    assert copper is not None, "Could not find COPPER CREEK on page 17"
    assert (
        copper.water.strip() == "COPPER CREEK"
    ), f"Expected 'COPPER CREEK', got '{copper.water}'"
    assert "6-12" in copper.mu, f"Expected MU '6-12' in {copper.mu}"
    assert (
        "6-12" not in copper.water
    ), f"MU should not be in water name: '{copper.water}'"

    # Find DEENA CREEK (should not have 6-12 in the name)
    deena = next((r for r in rows if "DEENA CREEK" in r.water), None)
    assert deena is not None, "Could not find DEENA CREEK on page 17"
    assert (
        deena.water.strip() == "DEENA CREEK"
    ), f"Expected 'DEENA CREEK', got '{deena.water}'"
    assert "6-12" in deena.mu, f"Expected MU '6-12' in {deena.mu}"
    assert "6-12" not in deena.water, f"MU should not be in water name: '{deena.water}'"
...
def test_region_metadata_extraction(parser, pdf, page_num, expected_region):
    ...
    assert (
        metadata.region == expected_region
...
def test_row_extraction_integrity(parser, pdf, page_num):
    ...
    for row in rows:
        ...
        assert (
            hasattr(row, "symbols") and hasattr(row, "page") and hasattr(row, "image")
        )
        assert isinstance(row.symbols, list)
        # raw_regs should be a string (not parsed yet)
        assert isinstance(
            row.raw_regs, str
        ), f"raw_regs for {row.water} is not a string"


TOTAL_PAGES = 88
EMPTY_PAGES = [
    p for p in range(1, TOTAL_PAGES + 1) if p not in PAGES_WITH_TABLES and p != 6
]
...
def test_region_name_normalization():
    ...
    for input_name, expected_output in test_cases:
        ...
        ), f"Failed: {input_name!r} -> {actual!r} (expected {expected_output!r})"


def test_kootenay_river_waterbody_name_cleaning(parser, pdf):
    """
    Test that KOOTENAY RIVER (upstream of Koocanusa Reservoir) on page 39
    has extra numbers removed from the waterbody name.
    The raw extracted text is "1 KOOTENAY RIVER 2 (upstream of Koocanusa Reservoir) 4 5"
    but the water name should be "KOOTENAY RIVER (upstream of Koocanusa Reservoir)".
    """
    # Page 39 is index 38
    page = pdf.pages[38]
    result = parser.extract_rows(page)
    ...
    # Find the Kootenay River (upstream) row
    kootenay = next(
        (r for r in rows if "KOOTENAY RIVER" in r.water and "Koocanusa" in r.water),
        None,
    )

    assert (
        kootenay is not None
    ), "Could not find KOOTENAY RIVER (upstream of Koocanusa Reservoir) on page 39"

    # The water name should NOT contain standalone numbers
    # Should be: "KOOTENAY RIVER (upstream of Koocanusa Reservoir)"
    # Should NOT be: "1 KOOTENAY RIVER 2 (upstream of Koocanusa Reservoir) 4 5"
    expected_name = "KOOTENAY RIVER (upstream of Koocanusa Reservoir)"
    assert (
        kootenay.water.strip() == expected_name
    ), f"Expected '{expected_name}', got '{kootenay.water}'"

    # Verify that the MUs are correctly extracted (these should NOT be in the water name)
    expected_mus = ["4-2", "4-21", "4-22", "4-24", "4-25", "4-35"]
    for mu in expected_mus:
        assert mu in kootenay.mu, f"Missing MU {mu} in extracted list: {kootenay.mu}"

    # Verify that the regulation text is present
    assert raw_regs_contain(
        kootenay, "Bait ban all year"
    ), f"Expected 'Bait ban all year' in raw regulations for Kootenay River"
    assert raw_regs_contain(
