...
- stream_tributary_of and lake_tributary_of attributes should be populated for all edges
- Lake tributaries should be detected via lake_tributary_of field
...
from fwa_pipeline.graph_builder import FWAPrimalGraph
...
class TestGuichonGraphTributaries:
        ...
        for stream_id in test_streams:
            ...
            for u, v, key, data in G.edges(keys=True, data=True):
                if data.get("linear_feature_id") == stream_id:
                    ...
                    logger.info(f"  lake_tributary_of: {lake_tributary_of}")
                    logger.info(f"  LAKE_NAME: {lake_name}")

                    # Should be tributary of Mamit Lake
                    assert (
                        lake_tributary_of == "Mamit Lake"
                    ), f"Stream {stream_id}: Expected 'Mamit Lake', got '{lake_tributary_of}'"
                    break
    ...
    def test_guichon_outlet_not_tributary_of_mamit(self, guichon_graph):
        ...
        for u, v, key, data in G.edges(keys=True, data=True):
            if data.get("linear_feature_id") == stream_id:
                ...
                assert lake_tributary_of != "Mamit Lake", (
                    f"Outlet stream incorrectly marked as tributary of Mamit Lake. "
                    f"lake_tributary_of={lake_tributary_of}"
                )
                break
    ...
    def test_guichon_main_stem_through_lake(self, guichon_graph):
        ...
        for segment in guichon_in_mamit:
            trib_of = segment["lake_tributary_of"]
            assert trib_of == "Mamit Lake", (
                f"Guichon Creek segment in Mamit Lake has lake_tributary_of='{trib_of}', "
    ...
    def test_dupuis_creek_tributary_of_guichon_not_mamit(self, guichon_graph):
        ...
        for u, v, key, data in G.edges(keys=True, data=True):
            if data.get("linear_feature_id") == stream_id:
                ...
                logger.info(f"\nStream {stream_id}:")
                logger.info(f"  GNIS_NAME: {gnis_name}")
                logger.info(f"  stream_tributary_of: {stream_tributary_of}")

                # Should be tributary of Dupuis Creek (not Mamit Lake)
                assert (
                    stream_tributary_of == "Dupuis Creek"
                ), f"Expected 'Dupuis Creek', got '{stream_tributary_of}'"
                break
    ...
    def test_named_streams_stop_lake_propagation(self, guichon_graph):
        ...
        for u, v, key, data in G.edges(keys=True, data=True):
            ...
            if (
                ...
                named_tributaries.append(
                    {
                        "name": gnis_name,
                        "stream_tributary_of": stream_tributary_of,
                        "linear_feature_id": data.get("linear_feature_id", ""),
                    }
                )
        ...
        if sample_size > 0:
            ...
            for trib in named_tributaries[:sample_size]:
                name = trib["name"]
                trib_of = trib["stream_tributary_of"]
    ...
    def test_all_edges_have_tributary_of(self, guichon_graph):
        """Test that all edges have the stream_tributary_of and lake_tributary_of attributes."""
        G = guichon_graph.G

        edges_with_stream_tributary = 0
        edges_with_lake_tributary = 0
        edges_without_stream_tributary = 0
        edges_without_lake_tributary = 0

        for u, v, key, data in G.edges(keys=True, data=True):
            if "stream_tributary_of" in data:
                edges_with_stream_tributary += 1
            else:
                edges_without_stream_tributary += 1

            if "lake_tributary_of" in data:
                edges_with_lake_tributary += 1
            else:
                edges_without_lake_tributary += 1

        logger.info(f"Edges with stream_tributary_of: {edges_with_stream_tributary:,}")
        logger.info(f"Edges with lake_tributary_of: {edges_with_lake_tributary:,}")

        total_edges = G.number_of_edges()
        assert (
            edges_with_stream_tributary == total_edges
        ), f"Only {edges_with_stream_tributary}/{total_edges} edges have stream_tributary_of attribute"

        assert (
            edges_with_lake_tributary == total_edges
        ), f"Only {edges_with_lake_tributary}/{total_edges} edges have lake_tributary_of attribute"

        logger.info("✓ All edges have both tributary attributes")
    ...
    def test_tributary_of_values_distribution(self, guichon_graph):
        ...
        stream_tributary_counts = {}
        lake_tributary_counts = {}

        for u, v, key, data in G.edges(keys=True, data=True):
            stream_trib_of = data.get("stream_tributary_of", "")
            lake_trib_of = data.get("lake_tributary_of", "")

            if stream_trib_of:
                stream_tributary_counts[stream_trib_of] = (
                    stream_tributary_counts.get(stream_trib_of, 0) + 1
                )

            if lake_trib_of:
                lake_tributary_counts[lake_trib_of] = (
                    lake_tributary_counts.get(lake_trib_of, 0) + 1
                )

        logger.info("\nStream tributary distribution:")
        for trib_name, count in sorted(
            stream_tributary_counts.items(), key=lambda x: -x[1]
        )[:20]:
            logger.info(f"  {trib_name}: {count} edges")

        logger.info("\nLake tributary distribution:")
        for trib_name, count in sorted(
            lake_tributary_counts.items(), key=lambda x: -x[1]
        )[:20]:
            logger.info(f"  {trib_name}: {count} edges")

        # Check that Mamit Lake has some tributaries
        mamit_count = lake_tributary_counts.get("Mamit Lake", 0)
    ...
    def test_lake_context_prevents_name_override(self, guichon_graph):
        ...
        for u, v, key, data in G.edges(keys=True, data=True):
            if data.get("lake_tributary_of") == "Mamit Lake":
        ...
        for u, v, key in mamit_tributary_edges[:10]:  # Check first 10
            ...
            for pred_u in G.predecessors(u):
                for pred_key in G[pred_u][u]:
                    ...
                    pred_lake_trib_of = pred_data.get("lake_tributary_of", "")
                    pred_name = pred_data.get("gnis_name", "")

                    # If unnamed and upstream of a Mamit tributary, should also be Mamit tributary
                    if not pred_name and pred_lake_trib_of == "Mamit Lake":
                        inheritance_working += 1
                    elif not pred_name and pred_lake_trib_of != "Mamit Lake":
    ...
    def test_export_graph_for_manual_inspection(self, guichon_graph):
        ...
        output_dir = Path(__file__).parent.parent / "fwa_modules" / "output"
